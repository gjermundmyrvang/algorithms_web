function showCode(event) {
  ...
  codeContainers.forEach(function(codeContainer) {
    if (codeContainer === event.currentTarget) {
      codeContainer.classList.toggle("show"); // Toggle klassen på den aktuelle codeContainer
    } else {
      codeContainer.classList.remove("show"); // Fjern "show" klassen fra alle andre codeContainere
    }
  });
...
codeContainers.forEach(function(codeContainer) {
  codeContainer.addEventListener("click", showCode);
});
...
function copyCode() {
    ...
    navigator.clipboard.writeText(codeText).then(function() {
      ...
      alert("Noe gikk galt ved kopieringen av koden.");
    });
  }
...
function showQuestions(index) {
  ...
  if (questions[index].img != null) {
    ...
    currentImg.setAttribute("src", `bilder/bigO/${codeImg}`);
  }
